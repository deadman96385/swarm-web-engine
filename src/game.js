...
export class Game {
  ...
  drawTowerSprite(type,level,x,y,size=48){const g=this.ctx,sy=TOWER_ROWS[type]??64;if(!this.assets.towers)return;g.save();g.globalCompositeOperation='lighter';g.drawImage(this.assets.towers,0,sy,64,64,x-size/2,y-size/2,size,size);if(type!=='POP')g.drawImage(this.assets.towers,Math.min(7,level)*64,sy,64,64,x-size/2,y-size/2,size,size);g.restore();}
  ...
  drawNativeTowerBar(t){
    ...
    if(t.level<7){const price=upgradeCost(t.cost,t.level),enabled=this.cash>=price;g.globalAlpha=enabled?1:.28;if(this.assets.towers){g.save();g.globalCompositeOperation='lighter';g.drawImage(this.assets.towers,3*64,0,64,64,304,698,96,96);g.restore();}g.fillStyle='#fff';g.font='700 11px system-ui';g.textAlign='center';g.fillText(`$${price}`,352,770);g.globalAlpha=1;}
    if(this.assets.towers){g.save();g.globalCompositeOperation='lighter';g.drawImage(this.assets.towers,(t.type==='POP'?4:2)*64,0,64,64,392,698,96,96);g.restore();}if(t.type!=='POP'){g.fillStyle='#fff';g.font='700 11px system-ui';g.textAlign='center';g.fillText(`$${Math.floor(t.value/2)}`,440,770);}
  }
