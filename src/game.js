...
export class Game {
  ...
  update(dt){
    ...
    // The native scene advances existing debris before towers and shots. Besides
    // matching its gravity-well timing, this lets a newly-created burst retain
    // its radial first-frame streak instead of consuming it immediately.
    this.updateParticles(dt);for(const t of this.towers.values())this.updateTower(t,dt);this.updateProjectiles(dt);
  ...
  breach(c){if(c.dead)return;c.dead=true;c.breached=true;this.lives--;this.boomAt(c.x,c.y,128,100,2,'#ff7fff');this.boomAt(c.x,c.y,128,250,2,'#ff00ff');this.boomAt(c.x,c.y,1024,500,2,'#ff003f');this.addLight(c.x,c.y,256,'#ffffff',1,.1,.5,'flareRandom');this.sound('life');if(this.lives>0&&this.lives<=10)this.sound(`countdown${this.lives}`);}
  ...
  addBackdropBoom(x,y,force,range){const p=this.camera?this.camera.worldToScreen({x,y}):{x,y},procedural=!this.assets.backdrop;this.backdrop.boomAt(p.x,p.y,force,range*(this.camera?.zoom??1),procedural);}
  ...
  boomAt(x,y,count,impulse,seconds,color=null,glow=true){const speed=impulse*1.5;this.explosionHue=(this.explosionHue+.02)%1;const tint=color??`hsl(${Math.round(this.explosionHue*360)} 100% 50%)`;this.spawnParticles(x,y,count*2,tint,{speedMin:speed,speedMax:speed,lifeMin:seconds,lifeMax:seconds,offsetTimeMax:.2,fadeFraction:.5,nativeBurst:true});if(glow){const backdropRange=Math.min(150,speed+speed*(Math.random()-.5)/2),lightLife=seconds*.1;this.addLight(x,y,speed,tint,lightLife,lightLife*.1,lightLife*.25,'random',false,Math.min(.75,speed/500));this.addBackdropBoom(x,y,backdropRange/2,backdropRange);}}
  spawnParticles(x,y,count,color='#d8f8ff',options={}){const {speedMin=25,speedMax=200,lifeMin=.6,lifeMax=2.2,massless=false,size=1,heading=null,spread=Math.PI,offsetTimeMax=0,fadeFraction=.5,nativeBurst=false}=options;for(let i=0;i<count&&this.particles.length<16383;i++){let nx,ny;if(nativeBurst&&heading==null){nx=Math.random()-.5;ny=Math.random()-.5;const length=Math.hypot(nx,ny)||1;nx/=length;ny/=length;}else{const angle=heading==null?Math.random()*Math.PI*2:heading+(Math.random()*2-1)*spread;nx=Math.cos(angle);ny=Math.sin(angle);}const speed=speedMin+Math.random()*(speedMax-speedMin),life=lifeMin+Math.random()*(lifeMax-lifeMin),vx=nx*speed,vy=ny*speed,offset=Math.random()*offsetTimeMax,px=x+vx*offset,py=y+vy*offset;this.particles.push({x:px,y:py,lastX:x,lastY:y,vx,vy,life,maxLife:life,fadeOut:life*fadeFraction,color,massless,size});}}
  sparkle(x,y,vx,vy,life,color,size=1){if(this.particles.length<16383)this.particles.push({x,y,lastX:x,lastY:y,vx,vy,life,maxLife:life,fadeOut:life*.25,color,massless:true,size});}
  updateParticles(dt){const bounds=this.camera?this.worldBounds:{minX:0,minY:0,maxX:480,maxY:800};for(const p of this.particles){p.life-=dt;if(p.life<=0){p.dead=true;continue;}if(!p.massless)for(const tower of this.towers.values()){if(tower.type!=='POP')continue;const center=hexCenter(...tower.cell),dx=center.x-p.x,dy=center.y-p.y,d=Math.hypot(dx,dy),range=towerRange('POP',tower.level);if(d>=range)continue;if(d<3){const max=250*tower.level;tower.energy+=1;if(tower.energy>max)tower.energy=max;else{const lightLife=Math.random()*.4;if(lightLife>0)this.addLight(center.x,center.y,Math.random()*64,`rgb(${Math.floor(Math.random()*64)} ${Math.floor(Math.random()*64)} ${Math.floor(Math.random()*64)})`,lightLife,lightLife*.25,lightLife*.25,'random');}p.dead=true;break;}const force=(1-d/range)*(2.5*tower.level)*100*dt;p.vx+=dx/d*force;p.vy+=dy/d*force;}if(!p.dead){p.lastX=p.x+.5;p.lastY=p.y;const drag=Math.pow(.95,dt*30);p.vx*=drag;p.vy*=drag;let nextX=p.x+p.vx*dt,nextY=p.y+p.vy*dt;if(nextX<=bounds.minX||nextX>=bounds.maxX){nextX=p.lastX;p.vx*=-1;}if(nextY<=bounds.minY||nextY>=bounds.maxY){nextY=p.lastY;p.vy*=-1;}p.x=nextX;p.y=nextY;}}this.particles=this.particles.filter(p=>!p.dead);}
  ...
  reward(c){
    ...
    this.boomAt(c.x,c.y,128,100,1,'#fffffc');
  ...
  draw(){
    ...
    this.drawBackdrop();this.drawPlayfield();
  ...
  drawBoardScene(){
    const g=this.ctx,procedural=!this.assets.honeycomb;if(this.assets.honeycomb&&!this.camera){g.globalAlpha=this.hexBrightness;g.drawImage(this.assets.honeycomb,0,0);g.globalAlpha=1;}else if(procedural||this.camera)this.drawHoneycombGrid();
    if(this.pathMode)this.drawPath();else{
    for(const key of this.level.blocked){const [q,r]=key.split(',').map(Number);this.hexFill(q,r,procedural?'rgba(205,48,37,.36)':'rgba(255,70,40,.25)','#ff412d');}for(const key of this.level.pass){const [q,r]=key.split(',').map(Number);this.hexFill(q,r,procedural?'rgba(238,126,24,.33)':'rgba(255,145,20,.22)','#ff9a24');}for(const key of this.level.fast??[]){const [q,r]=key.split(',').map(Number);this.hexFill(q,r,procedural?'rgba(0,166,218,.34)':'rgba(0,190,255,.28)','#28dfff');}for(const key of this.level.heal??[]){const [q,r]=key.split(',').map(Number);this.hexFill(q,r,procedural?'rgba(49,205,91,.34)':'rgba(70,255,110,.28)','#6dff7f');}
    if(this.camera){for(const s of this.level.spawns)this.drawLaneEndpoint(s.cell,s.laneIndex,true);let index=0;for(const e of this.level.exits.values())this.drawLaneEndpoint(e,index++,false);}else{for(const s of this.level.spawns)this.hexFill(...s.cell,procedural?'rgba(0,157,211,.34)':'rgba(0,170,255,.28)','#21d7ff');for(const e of this.level.exits.values())this.hexFill(...e,procedural?'rgba(92,213,46,.32)':'rgba(117,255,48,.24)','#8cff42');}
    }
  ...
  drawParticles(){const g=this.ctx,groups=new Map();for(const p of this.particles){const fadeOut=p.fadeOut??(p.maxLife??p.life)*.5,alpha=fadeOut>0&&p.life<fadeOut?p.life/fadeOut:1,bucket=Math.max(1,Math.round(alpha*16)),key=`${p.color}|${bucket}|${p.size??1}`;let group=groups.get(key);if(!group){group={color:p.color,alpha:bucket/16,size:p.size??1,particles:[]};groups.set(key,group);}group.particles.push(p);}g.save();g.globalCompositeOperation='lighter';g.lineCap='butt';for(const group of groups.values()){g.strokeStyle=group.color;g.lineWidth=Math.max(1,group.size);g.globalAlpha=group.alpha*.5;g.beginPath();for(const p of group.particles){g.moveTo(p.lastX??p.x+.5,p.lastY??p.y);g.lineTo(p.x,p.y);}g.stroke();g.fillStyle=group.color;g.globalAlpha=group.alpha;g.beginPath();const head=Math.max(1,group.size*.65);for(const p of group.particles)g.rect(p.x-head/2,p.y-head/2,head,head);g.fill();}g.restore();}
  ...
  drawNativeInterface(){
    ...
    if(this.assets.scoreBar)g.drawImage(this.assets.scoreBar,0,0,480,32);else this.drawProceduralScoreBar();
  ...
  drawProceduralScoreBar(){const g=this.ctx,bar=g.createLinearGradient(0,0,0,32);g.save();bar.addColorStop(0,'rgba(0,91,96,.96)');bar.addColorStop(.38,'rgba(0,44,55,.94)');bar.addColorStop(1,'rgba(1,12,22,.97)');g.fillStyle=bar;g.fillRect(0,0,480,32);g.globalCompositeOperation='lighter';g.strokeStyle='rgba(89,229,239,.16)';g.lineWidth=1;for(let x=-20;x<500;x+=64){g.beginPath();g.moveTo(x,0);g.lineTo(x+12,10);g.lineTo(x+52,10);g.lineTo(x+64,0);g.stroke();}const rim=g.createLinearGradient(0,25,0,32);rim.addColorStop(0,'rgba(45,238,244,0)');rim.addColorStop(1,'rgba(45,238,244,.34)');g.fillStyle=rim;g.fillRect(0,25,480,7);g.strokeStyle='rgba(88,248,255,.38)';g.beginPath();g.moveTo(0,31.5);g.lineTo(480,31.5);g.stroke();g.restore();}
  traceProceduralBottomBoundary(g,top=646){g.beginPath();g.moveTo(0,top+1);for(let x=0;x<480;x+=72){g.lineTo(Math.min(480,x+18),top-4);g.lineTo(Math.min(480,x+36),top+1);g.lineTo(Math.min(480,x+54),top-6);g.lineTo(Math.min(480,x+72),top+1);}}
  drawProceduralTrayGrid(top=638){const g=this.ctx,radius=11,height=Math.sqrt(3)*radius,step=radius*1.5;g.save();g.globalCompositeOperation='lighter';g.globalAlpha=.18;g.strokeStyle='#3aa9d0';g.lineWidth=.8;for(let q=-1;q*step-radius<480;q++)for(let row=-1;;row++){const x=6+q*step,y=top+height/2+row*height+(q%2?height/2:0);if(y-radius>800)break;if(y+radius<top)continue;this.traceHex(g,x,y,radius);g.stroke();}g.restore();}
  drawProceduralCashDock(){const g=this.ctx;g.save();g.fillStyle='rgba(2,19,35,.92)';g.strokeStyle='rgba(77,244,213,.78)';g.lineWidth=1.5;g.beginPath();g.moveTo(186,650);g.lineTo(198,659);g.lineTo(282,659);g.lineTo(294,650);g.lineTo(302,658);g.lineTo(290,686);g.lineTo(190,686);g.lineTo(178,658);g.closePath();g.fill();g.globalCompositeOperation='lighter';g.globalAlpha=.18;g.lineWidth=8;g.stroke();g.globalAlpha=.9;g.lineWidth=1.5;g.stroke();g.restore();}
  drawProceduralBottomBar(){const g=this.ctx,top=646,bottom=800;g.save();g.beginPath();g.rect(0,top-8,480,bottom-top+8);g.clip();this.traceProceduralBottomBoundary(g,top);g.lineTo(480,bottom);g.lineTo(0,bottom);g.closePath();const tray=g.createLinearGradient(0,top-8,0,bottom);tray.addColorStop(0,'rgba(0,61,82,.90)');tray.addColorStop(.18,'rgba(0,44,70,.88)');tray.addColorStop(1,'rgba(0,24,47,.95)');g.fillStyle=tray;g.fill();this.drawProceduralTrayGrid(top-8);this.traceProceduralBottomBoundary(g,top);g.globalCompositeOperation='lighter';g.lineJoin='round';g.strokeStyle='#42ff89';g.globalAlpha=.10;g.lineWidth=13;g.stroke();this.traceProceduralBottomBoundary(g,top);g.strokeStyle='#33f5b0';g.globalAlpha=.28;g.lineWidth=6;g.stroke();this.traceProceduralBottomBoundary(g,top);g.strokeStyle='#72f8ff';g.globalAlpha=.92;g.lineWidth=1.75;g.stroke();g.restore();this.drawProceduralCashDock();}
  drawProceduralPauseGlyph(){const g=this.ctx,cx=452,cy=689;g.save();g.fillStyle='rgba(1,18,32,.74)';g.beginPath();g.arc(cx,cy,18,0,Math.PI*2);g.fill();g.globalCompositeOperation='lighter';g.fillStyle='#8ffaff';g.globalAlpha=.16;g.beginPath();g.arc(cx,cy,22,0,Math.PI*2);g.fill();g.globalAlpha=.95;g.fillRect(cx-7,cy-10,4,20);g.fillRect(cx+3,cy-10,4,20);g.globalAlpha=.46;g.strokeStyle='#8ffaff';g.lineWidth=1.5;g.beginPath();g.arc(cx,cy,18,0,Math.PI*2);g.stroke();g.restore();}
  drawNativeBuildBar(){
    const g=this.ctx,procedural=!this.assets.buildBar;if(!procedural)g.drawImage(this.assets.buildBar,0,646,480,154);else this.drawProceduralBottomBar();
  ...
  drawNativeTowerBar(t){
    ...
    if(this.assets.towerBar)g.drawImage(this.assets.towerBar,0,646,480,154);else this.drawProceduralBottomBar();g.fillStyle='#dfffff';g.font='700 13px system-ui';g.textAlign='center';g.fillText(`$${this.cash}`,240,676);this.drawTowerSprite(t.type,t.level,40,755,64);g.textAlign='left';g.fillStyle='#fff';g.font='700 14px system-ui';g.fillText(name,10,710);g.fillStyle='#b7d4df';g.font='12px system-ui';this.drawWrapped(desc,75,734,220,13,3);
  ...
  traceHex(g,x,y,radius=24){g.beginPath();for(let i=0;i<6;i++){const a=Math.PI/3*i,xp=x+Math.cos(a)*radius,yp=y+Math.sin(a)*radius;i?g.lineTo(xp,yp):g.moveTo(xp,yp);}g.closePath();}
  hexFill(q,r,fill,stroke){const {x,y}=hexCenter(q,r),g=this.ctx;this.traceHex(g,x,y);if(this.assets.honeycomb){g.fillStyle=fill;g.fill();g.strokeStyle=stroke;g.lineWidth=2;g.stroke();return;}g.save();g.fillStyle=fill;g.fill();g.save();this.traceHex(g,x,y);g.clip();const sheen=g.createLinearGradient(0,y-24,0,y+24);sheen.addColorStop(0,'rgba(255,255,255,.08)');sheen.addColorStop(.48,'rgba(255,255,255,.018)');sheen.addColorStop(1,'rgba(0,0,0,.08)');g.fillStyle=sheen;g.fillRect(x-24,y-24,48,48);g.restore();g.globalCompositeOperation='lighter';g.strokeStyle=stroke;g.globalAlpha=.18;g.lineWidth=7;this.traceHex(g,x,y);g.stroke();g.globalAlpha=.94;g.lineWidth=1.75;this.traceHex(g,x,y);g.stroke();g.restore();}
  drawHoneycombGrid(){const g=this.ctx,procedural=!this.assets.honeycomb,alpha=Math.max(procedural?.30:.06,(this.hexBrightness??.25)*(procedural?.95:.5));g.save();g.globalCompositeOperation='lighter';g.strokeStyle=procedural?'#31b696':'#1f6f9a';if(procedural){g.globalAlpha=alpha*.38;g.lineWidth=5;for(let q=0;q<this.grid.cols;q++)for(let r=0;r<this.grid.rows;r++){const {x,y}=hexCenter(q,r);this.traceHex(g,x,y);g.stroke();}}g.globalAlpha=alpha;g.lineWidth=procedural?1.6:1;for(let q=0;q<this.grid.cols;q++)for(let r=0;r<this.grid.rows;r++){const {x,y}=hexCenter(q,r);this.traceHex(g,x,y);g.stroke();}g.restore();}
  drawProceduralTowerBase(col,r){const g=this.ctx;g.save();const halo=g.createRadialGradient(0,0,r*.15,0,0,r*1.55);halo.addColorStop(0,col);halo.addColorStop(.38,col);halo.addColorStop(1,'rgba(0,0,0,0)');g.globalCompositeOperation='lighter';g.globalAlpha=.14;g.fillStyle=halo;g.fillRect(-r*1.6,-r*1.6,r*3.2,r*3.2);g.globalCompositeOperation='source-over';g.globalAlpha=1;g.fillStyle='rgba(3,11,24,.82)';g.beginPath();g.arc(0,0,r,0,Math.PI*2);g.fill();g.globalCompositeOperation='lighter';g.strokeStyle=col;g.globalAlpha=.22;g.lineWidth=Math.max(4,r*.3);g.stroke();g.globalAlpha=.96;g.lineWidth=Math.max(1.5,r*.11);g.stroke();g.globalAlpha=.42;g.lineWidth=1;g.beginPath();g.arc(0,0,r*.67,0,Math.PI*2);g.stroke();g.restore();}
  drawProceduralTowerMotif(type,col,r){const g=this.ctx;g.save();g.globalCompositeOperation='lighter';g.strokeStyle=col;g.fillStyle=col;g.lineCap='round';g.lineJoin='round';g.lineWidth=Math.max(1.4,r*.12);if(type==='BLASTER'){for(const x of [-r*.25,r*.25]){g.beginPath();g.moveTo(x,r*.25);g.lineTo(x,-r*.68);g.stroke();}g.beginPath();g.arc(0,r*.3,r*.2,0,Math.PI*2);g.fill();}else if(type==='LASER'){g.beginPath();g.moveTo(0,r*.45);g.lineTo(0,-r*.78);g.moveTo(-r*.38,-r*.08);g.lineTo(r*.38,-r*.08);g.stroke();g.beginPath();g.arc(0,-r*.08,r*.16,0,Math.PI*2);g.fill();}else if(type==='MISSILE'){g.beginPath();g.moveTo(0,-r*.78);g.lineTo(r*.38,r*.35);g.lineTo(0,r*.12);g.lineTo(-r*.38,r*.35);g.closePath();g.stroke();g.beginPath();g.moveTo(0,r*.12);g.lineTo(0,r*.58);g.stroke();}else if(type==='SHOCK'){g.beginPath();g.moveTo(r*.16,-r*.75);g.lineTo(-r*.22,-r*.08);g.lineTo(r*.22,-r*.08);g.lineTo(-r*.18,r*.7);g.stroke();}else if(type==='THUMP'){g.lineWidth=Math.max(2,r*.18);g.beginPath();g.moveTo(-r*.62,0);g.lineTo(r*.62,0);g.moveTo(0,-r*.62);g.lineTo(0,r*.62);g.stroke();g.lineWidth=1;g.strokeRect(-r*.25,-r*.25,r*.5,r*.5);}else if(type==='POP'){for(let i=0;i<4;i++){g.rotate(Math.PI/2);g.beginPath();g.moveTo(0,-r*.3);g.lineTo(-r*.2,-r*.7);g.lineTo(r*.2,-r*.7);g.closePath();g.stroke();}g.beginPath();g.arc(0,0,r*.2,0,Math.PI*2);g.fill();}else{g.beginPath();g.moveTo(0,r*.4);g.lineTo(0,-r*.7);g.stroke();}g.restore();}
  drawTowerGlyph(type,x,y,size){const g=this.ctx,col=TOWER_STATS[type]?.color??'#fff',r=size*.32;g.save();g.translate(x,y);this.drawProceduralTowerBase(col,r);this.drawProceduralTowerMotif(type,col,r);g.restore();}
  drawTowerVector(t,p){const g=this.ctx,col=TOWER_STATS[t.type]?.color??'#fff',r=15;g.save();g.translate(p.x,p.y);this.drawProceduralTowerBase(col,r);g.save();if(['BLASTER','LASER','MISSILE'].includes(t.type))g.rotate((t.heading??0)+Math.PI/2);else if(t.type==='SHOCK'||t.type==='POP')g.rotate(t.rotation??0);else if(t.type==='THUMP')g.rotate(Math.PI/4);this.drawProceduralTowerMotif(t.type,col,r);g.restore();g.globalCompositeOperation='lighter';g.fillStyle=col;g.globalAlpha=.86;const lv=Math.min(7,t.level);for(let i=0;i<lv;i++){const a=-Math.PI/2+i/7*Math.PI*2;g.beginPath();g.arc(Math.cos(a)*19,Math.sin(a)*19,1.5,0,Math.PI*2);g.fill();}g.restore();}
  paintProceduralCreepPath(col,size,fillAlpha=.28){const g=this.ctx;g.save();g.globalCompositeOperation='source-over';g.fillStyle='rgba(3,8,20,.72)';g.fill();g.globalCompositeOperation='lighter';g.strokeStyle=col;g.globalAlpha=.16;g.lineWidth=Math.max(4,size*.22);g.stroke();g.fillStyle=col;g.globalAlpha=fillAlpha;g.fill();g.globalAlpha=.96;g.lineWidth=Math.max(1.2,size*.075);g.stroke();g.restore();}
  drawCreepVector(c,size){const g=this.ctx,col=CREEP_COLORS[c.type]??'#ff5bda',rot=c.rotation??0,r=size/2;g.save();g.translate(c.x,c.y);const poly=pts=>{g.beginPath();pts.forEach(([x,y],i)=>i?g.lineTo(x,y):g.moveTo(x,y));g.closePath();},accent=()=>{g.globalCompositeOperation='lighter';g.strokeStyle='#e5ffff';g.globalAlpha=.52;g.lineWidth=Math.max(1,size*.04);};switch(c.type){case 'CHOMPER':g.rotate(rot);poly([[r,0],[-r*.8,r*.8],[-r*.35,0],[-r*.8,-r*.8]]);this.paintProceduralCreepPath(col,size,.3);accent();g.beginPath();g.moveTo(-r*.42,0);g.lineTo(r*.22,0);g.stroke();break;case 'SPINNER':g.rotate(rot*3);for(let i=0;i<4;i++){g.rotate(Math.PI/2);poly([[0,-r],[r*.28,-r*.3],[0,-r*.1],[-r*.28,-r*.3]]);this.paintProceduralCreepPath(col,size,.3);}g.globalCompositeOperation='lighter';g.fillStyle=col;g.globalAlpha=.9;g.beginPath();g.arc(0,0,r*.18,0,Math.PI*2);g.fill();break;case 'WIGGLE':{g.rotate(rot);g.beginPath();for(let i=0;i<=24;i++){const a=i/24*Math.PI*2,rr=r*(1+.26*Math.sin(a*3+this.visualTime*6)),x=Math.cos(a)*rr*1.15,y=Math.sin(a)*rr*.7;i?g.lineTo(x,y):g.moveTo(x,y);}g.closePath();this.paintProceduralCreepPath(col,size,.26);accent();g.beginPath();g.ellipse(0,0,r*.5,r*.25,0,0,Math.PI*2);g.stroke();break;}case 'STAR':g.rotate(rot);g.beginPath();for(let i=0;i<10;i++){const a=Math.PI/5*i-Math.PI/2,rr=i%2?r*.45:r,x=Math.cos(a)*rr,y=Math.sin(a)*rr;i?g.lineTo(x,y):g.moveTo(x,y);}g.closePath();this.paintProceduralCreepPath(col,size,.26);accent();g.beginPath();g.arc(0,0,r*.24,0,Math.PI*2);g.stroke();break;case 'CUBIC':g.rotate(rot);g.beginPath();g.rect(-r*.8,-r*.8,r*1.6,r*1.6);this.paintProceduralCreepPath(col,size,.28);accent();g.rotate(Math.PI/4);g.strokeRect(-r*.48,-r*.48,r*.96,r*.96);break;case 'PULSAR':{const pulse=.5+.5*Math.sin(this.visualTime*6);g.globalCompositeOperation='source-over';g.fillStyle='rgba(3,8,20,.72)';g.beginPath();g.arc(0,0,r*.42,0,Math.PI*2);g.fill();g.globalCompositeOperation='lighter';g.strokeStyle=col;g.lineWidth=Math.max(1.4,size*.09);for(let k=0;k<3;k++){g.globalAlpha=(1-k*.24)*(.55+.45*pulse);g.beginPath();g.arc(0,0,r*(.38+k*.28+pulse*.12),0,Math.PI*2);g.stroke();}g.fillStyle=col;g.globalAlpha=.72;g.beginPath();g.arc(0,0,r*.18,0,Math.PI*2);g.fill();break;}case 'SWARM':g.rotate(rot*4);poly([[0,-r],[r*.85,r*.6],[-r*.85,r*.6]]);this.paintProceduralCreepPath(col,size,.3);accent();poly([[0,-r*.48],[r*.4,r*.28],[-r*.4,r*.28]]);g.stroke();break;default:g.beginPath();g.arc(0,0,r,0,Math.PI*2);this.paintProceduralCreepPath(col,size,.3);}g.restore();}
