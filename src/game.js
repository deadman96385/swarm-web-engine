...
export class Game {
  ...
  addBackdropBoom(x,y,force,range){const p=this.camera?this.camera.worldToScreen({x,y}):{x,y},procedural=!this.assets.backdrop,spread=procedural?1.4:1;this.backdrop.boomAt(p.x,p.y,force,range*spread*(this.camera?.zoom??1),procedural);}
  ...
  drawBoardScene(){
    const g=this.ctx;if(this.assets.honeycomb&&!this.camera){g.globalAlpha=this.hexBrightness;g.drawImage(this.assets.honeycomb,0,0);g.globalAlpha=1;}else if(this.camera)this.drawHoneycombGrid();
  ...
  drawProceduralBottomBar(){const g=this.ctx,top=646,bottom=800;g.save();g.beginPath();g.rect(0,top,480,bottom-top);g.clip();g.fillStyle='rgba(0,40,66,.68)';g.fillRect(0,top,480,bottom-top);g.globalCompositeOperation='lighter';const edge=g.createLinearGradient(0,top,0,top+24);edge.addColorStop(0,'rgba(39,242,255,.34)');edge.addColorStop(1,'rgba(39,242,255,0)');g.fillStyle=edge;g.fillRect(0,top,480,24);g.strokeStyle='rgba(42,244,255,.72)';g.lineWidth=2;g.beginPath();g.moveTo(0,top+1);g.lineTo(480,top+1);g.stroke();g.restore();}
  drawNativeBuildBar(){
    const g=this.ctx;if(this.assets.buildBar)g.drawImage(this.assets.buildBar,0,646,480,154);else this.drawProceduralBottomBar();
