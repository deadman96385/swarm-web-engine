...
test('XL camera converts placement input and empty-board drags pan the world',()=>{globalThis.requestAnimationFrame=()=>1;globalThis.cancelAnimationFrame=noOp;const level=testLevel();level.grid={cols:22,rows:24};level.xl=true;level.spawns[0].exit=[21,0];level.exits=new Map([['e1',[21,0]]]);const game=new Game(canvasStub(),level,{}),cell=[10,11],screen=game.camera.worldToScreen(hexCenter(...cell));game.setBuildPreview('BLASTER',screen.x,screen.y);assert.deepEqual(game.buildPreview.cell,cell);const before=game.camera.centerX,pointer=(x,y)=>({clientX:x,clientY:y,pointerId:7,pointerType:'mouse'});game.onPointer(pointer(240,300));game.onPointerMove(pointer(280,300));game.onPointerUp(pointer(280,300));assert.ok(game.camera.centerX<before);game.destroy();});

test('XL minimap stops intercepting input when the fitted board is fully visible',()=>{globalThis.requestAnimationFrame=()=>1;globalThis.cancelAnimationFrame=noOp;const level=testLevel();level.grid={cols:22,rows:24};level.xl=true;level.spawns[0].exit=[21,0];level.exits=new Map([['e1',[21,0]]]);const game=new Game(canvasStub(),level,{});assert.equal(game.handleMinimap(400,60),true);game.camera.fit();assert.equal(game.camera.boardFullyVisible(),true);assert.equal(game.handleMinimap(400,60),false);game.destroy();});
