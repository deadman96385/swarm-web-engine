...
test('place-above-touch applies the native 96 pixel build offset',()=>{globalThis.requestAnimationFrame=()=>1;globalThis.cancelAnimationFrame=noOp;const game=new Game(canvasStub(),testLevel(),{}, {},null,{hardcore:false,placeAboveFinger:true}),{x,y}=hexCenter(5,0);game.setBuildPreview('BLASTER',x,y+96);assert.deepEqual(game.buildPreview.cell,[5,0]);game.destroy();});
