...
test('authored XL scores persist without farming original Swarm achievements',()=>{const profile=new ProfileStore(new MemoryStorage());for(let i=1;i<=12;i++)profile.record({sourceName:`XL/GAME_LEVEL_XL_E_${String(i).padStart(4,'0')}.xml`,name:`XL ${i}`,difficulty:'Easy',campaign:'xl',lives:15,endless:false},result(i*100,15));assert.equal(profile.leaderboard().length,12);const earned=new Set(profile.data.achievements);assert.ok(!earned.has('E1')&&!earned.has('E2')&&!earned.has('E3'));});
